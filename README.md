...
> ⚠️ **TODO**: Re-run Exp 15 with proper full-text search workload:
> - Data: 1M+ documents with paragraph-length text (100-500 words per doc)
> - Queries: multi-term search, phrase match, fuzzy/typo-tolerant, wildcard
> - Compare: OpenSearch inverted index vs PostgreSQL `tsvector` + `GIN` index
> - Bulk API for ingestion (not single-doc insert)
> - This benchmark only tested KV get-by-id — NOT OpenSearch's actual strength
